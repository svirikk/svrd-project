...
int main(int argc, char *argv[])
{
    if (argc != 10)
    {
        std::fprintf(stderr, "Usage: string_art input.pgm num_pins opacity threshold skipped_neighbors scale_factor min_steps max_steps output.pgm\n");
        return 0;
    }
    ...
    int minSteps = std::strtol(argv[7], nullptr, 10);
    int maxSteps = std::strtol(argv[8], nullptr, 10);
    std::FILE* outputFile = std::fopen(argv[9], "wb");

    // actual art being done
    StringArtist stringArtist = StringArtist(image, numPins, draftOpacity, threshold, skippedNeighbors, scaleFactor, minSteps, maxSteps);
