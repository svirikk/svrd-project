 ...
 *
 *  КРИТИЧНО: "uploads" — це ПОСТІЙНА спільна тека (multer пише туди
 *  вхідні фото до того, як вони потрапляють у власну сесію), а НЕ
 *  тимчасова сесія. Раніше вона не була виключена з цього циклу, і якщо
 *  довше maxAgeMs ніхто не завантажував фото (звичайна нічна пауза), її
 *  mtime переставав оновлюватись, і чистка видаляла ЇЇ ЦІЛКОМ як "застарілу
 *  сесію". Наступне завантаження фото тоді падало з 400 "Не вдалося
 *  завантажити файл." — бо multer намагався писати в теку, якої вже
 *  фізично не існувало. Саме це й спричиняло "Failed to fetch" після
 *  кількох годин затишшя. */
function cleanupStaleSessions(maxAgeMs = 2 * 60 * 60 * 1000) {
  fs.readdir(config.TMP_DIR, (err, entries) => {
    if (err) return;
    const now = Date.now();
    for (const entry of entries) {
      if (entry === '.gitkeep' || entry === 'uploads') continue;
